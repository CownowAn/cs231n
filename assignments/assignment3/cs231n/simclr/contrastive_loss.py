...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        # Compute denominators for l(k, k_N), l(k_N, k) 
        denominator_k = 0
        denominator_k_N = 0
        for i in range(2*N):
            if i != k:
        ...
        # Compute l(k, k_N)
        loss_k = -torch.log(torch.exp(sim(z_k, z_k_N) / tau) / denominator_k)
        
        # Compute l(k_N, k)
        loss_k_N = -torch.log(torch.exp(sim(z_k_N, z_k) / tau) / denominator_k_N)
...
def sim_positive_pairs(out_left, out_right):
    ...
    pos_pairs = torch.unsqueeze(pos_pairs, dim=1)
...
def compute_sim_matrix(out):
    ...
    out_normalized = out / (torch.unsqueeze(torch.linalg.norm(out, dim=1), dim=1))
...
def simclr_loss_vectorized(out_left, out_right, tau, device='cuda'):
    ...
    exponential = torch.exp(sim_matrix / tau).to(device) # [2*N, 2*N]
    
    # This binary mask zeros out terms where k=i.
    mask = (torch.ones_like(exponential, device=device) - torch.eye(2 * N, device=device)).to(device).bool()
    
    # We apply the binary mask.
    exponential = exponential.masked_select(mask).view(2 * N, -1).to(device)  # [2*N, 2*N-1]
    
    # Hint: Compute the denominator values for all augmented samples. This should be a 2N x 1 vector.
    denom = torch.sum(exponential, dim=1).to(device) # [2*N, 1]
    ...
    sim = sim_positive_pairs(out_left, out_right).to(device) # [N, 1]
    # similarities for k and k+N are same
    sim = torch.cat((sim, sim), dim=0).to(device) # [2*N, 1]
    ...
    numerator = torch.squeeze(torch.exp(sim / tau)).to(device)

    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    
    # Step 4: Now that you have the numerator and denominator for all augmented samples, compute the total loss.
    loss = torch.sum(-torch.log(numerator / denom), dim=0) / (2 * N)
